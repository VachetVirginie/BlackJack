...
function Player() {
	...
	this.getBank = function() {
		if(bank >= 0) {
			return $('#bank').html('<strong style="color: #00FF00">Gains: ' + bank + '</strong>');
        }
         else {
			return $('#bank').html('<strong style="color: #C00">Gains: -' + 
...
function getWinner() {
	...
	if(cash <= 0) {
		var newGame = confirm('Veux tu rejouer?');
...
$('#deal').on('click', function() {
	...
	if(cash > 0 && !running) {
		...
		var newGame = confirm('Tu as tout perdu veux tu rejouer?');
